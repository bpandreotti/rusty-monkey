...
fn assert_compile(input: &str, expected: code::Instructions) {
    let bytecode = test_utils::parse_and_compile(input)
...
fn test_conditionals() {
    assert_compile("if true { 10 }; 3333",
        Instructions([
            make!(OpCode::OpTrue),
    ...
    assert_compile("if true { 10 } else { 20 }; 3333",
        Instructions([
            make!(OpCode::OpTrue),
            make!(OpCode::OpJumpNotTruthy, 10),
            make!(OpCode::OpConstant, 0),
...
fn test_global_assignment() {
    assert_compile("let one = 1; let two = 2",
        Instructions([
            make!(OpCode::OpConstant, 0),
    ...
    assert_compile("let one = 1; let two = one; two",
        Instructions([
            make!(OpCode::OpConstant, 0),
            make!(OpCode::OpSetGlobal, 0),
            make!(OpCode::OpGetGlobal, 0),
